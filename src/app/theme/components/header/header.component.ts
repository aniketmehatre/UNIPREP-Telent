...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  private setupSubscriptions() {
    ...
    this.subs.sink = this.dataService.openReportWindowSource.subscribe({
      ...
      error: (err) =>
        console.error("Error in report window subscription:", err),
    });
    ...
    this.subs.sink = this.assessmentService.iLearnChallengeData$.subscribe(
      (data) => {
  ...
  loadUserData() {
    if (this.authService._user) {
      ...
      // Demo/Trial
      if (
        typeof user.login_status === "string" &&
        user.login_status.includes("Demo")
      ) {
        this.demoTrial = true;
  ...
  private updateHomeCountryState(country: any) {
    if (!country) return;
    this.headerHomeFlag =
      country.flag || `../../uniprep-assets/icons/india.png`;
  ...
  onChangeModuleList(moduleId: number = 1) {
    ...
    this.locationService
      .getSubModuleByModule({ moduleid: moduleId })
      .subscribe((res) => {
  ...
  onChangeSubModuleList(subModuleId: any = 1) {
    ...
    this.locationService.getModuleQuestionList(data).subscribe((res) => {
      this.questionList = res.questions.map((x: any, i: number) => ({
        ...x,
        index: i + 1,
      }));
    });
  ...
  getReportOption() {
    ...
    this.subs.sink = this.locationService
      .getReportOptionList()
      .subscribe((data) => {
        const reportTypeData = data.reportOptions.filter(
  ...
  onSubmit(op: Popover) {
    ...
    const data = {
      ...
      submoduleId:
        this.moduleQuestionReport?.subModuleId || formVal.submoduleId,
  ...
  checkNewUser(): void {
    this.authService.getNewUserTimeLeft().subscribe((res) => {
      ...
      if (data.plan === "on_progress") {
        ...
        this.userLoginTimeLeftCount = false;
        this.getTimer(data.minutes, data.seconds, data.hours, 50, data.months);
      }
    });
  }
  ...
  continueTrial(): void {
    let data: any = {};
    if (
      this.mobileForm.get("nationality_id")?.valid &&
      this.mobileForm.get("study_level")?.valid
    ) {
    ...
    this.dashboardService.getContineTrial(data).subscribe({
      next: (res) => {
        ...
        this.authService._user.nationality_id =
          this.mobileForm.value.nationality_id;
  ...
  onClickSubscribedUser(): void {
    const isStandardUser =
      this.imageWhiteLabelDomainName?.toLowerCase() === "uniprep" ||
      this.imageWhiteLabelDomainName === "Partner";
  ...
  showSearchComponent(type: any) {
    ...
    this.isShowHeaderSearchForModule =
      this.activeHeaderSearch.stage !== "questionsearch";
  }
  ...
  conditionModuleOrQuestionComponent() {
    ...
    this.showSearch = !hideSearchRoutes.some((route) =>
  ...
  markAsRead(notification: any) {
    ...
    this.dashboardService
      .userNotificationread(notification.id)
      .subscribe(() => {
  ...
  markAllRead(event: Event) {
    ...
    this.dashboardService
      .userNotificationread(undefined, "read_all")
      .subscribe(() => {
