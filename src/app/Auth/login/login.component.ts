    ...
    this.locationService
      ...
      .subscribe((data: any) => {
        ...
        if (data.brand_primary_color && data.brand_secondary_color) {
          this.brandColorService.fetchAndApplyColors(
        ...
        next: (response) => {
          ...
          this.service.getMe().subscribe({
            next: (userData) => {
              ...
              if (!userData.userdetails[0].city_id) {
                setTimeout(() => {
                  this.updateLocation();
                }, 3000);
              }
      ...
      .subscribe({
        ...
        error: (error) => {
          this.toast.add({
            ...
            summary: "Error",
            detail:
              error?.error?.message || error.message || "Social login failed",
          });
        },
      });
  }
