...
export class RegistrationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.position && this.jobId) {
        ...
        if (this.service.isTokenValid()) {
          this.router.navigate([
            `/pages/talent-connect/easy-apply/${this.jobId}`,
          ]);
        }
    ...
    // Google Sign-in: only on uniprep.ai – no subscribe and no popup on other domains
    if (this.isSocialLoginEnabled) {
      this.authService.authState.subscribe((user) => {
        if (!user || !this.isSocialLoginEnabled) return;
        this.googleResponse = user;
        this.googleResponse = {
          ...this.googleResponse,
          position: this.storage.get("position"),
          job_id: this.storage.get("jobId"),
        };
          ...
          error: (error: any) => {
            ...
            this.toast.add({
              ...
              detail:
                error?.error?.message ||
                error?.message ||
                "An unexpected error occurred, please contact the team",
            });
          },
        });
      });
  ...
  onSubmit() {
    ...
    if (
      this.registrationForm.value.password.length < 8 ||
      this.registrationForm.value.confirmPassword.length < 8
    ) {
      this.toast.add({
        severity: "error",
        summary: "Error",
