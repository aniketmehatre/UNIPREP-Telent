...
export class PitchDeskComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  bookmarkQuestion(courseId: any, isFav: any) {
    ...
    this.pitchDesk
      .bookmarkCourseData(courseId, this.PersonalInfo.user_id, isFav)
      .subscribe((response) => {
        let pitchListData = this.pitchDeskList.find(
          (item: any) => item.id == courseId,
        );
        isFav == true
          ? (pitchListData.favourite = 1)
          : (pitchListData.favourite = null);
        this.toast.add({
          severity: "success",
          summary: "Success",
  ...
  getPitchDeskList() {
    ...
    if (this.viewFavourites) {
      ...
      data = {
        country: this.filterForm.value.country
          ...
          : "",
        funding_type: this.filterForm.value.funding_type
          ? this.filterForm.value.funding_type
          : "",
        sector: this.filterForm.value.sector
          ? this.filterForm.value.sector
          : "",
        page: this.page,
        perpage: this.pageSize,
        planname: this.currentPlan ? this.currentPlan : "",
        pitchdeck_name: this.filterForm.value.pitchdeck_name
          ? this.filterForm.value.pitchdeck_name
          : "",
      };
    }
    this.pitchDesk.getPitchDeskData(data).subscribe((responce) => {
      ...
      this.selectedCheckboxCount = this.selectAllCheckboxes
        ? this.pitchDeskList.length
        : 0;
      this.exportCreditCount = responce.credit_count
        ? responce.credit_count
        : 0;
  ...
  onCheckboxChange(event: any, item: any) {
    ...
    this.selectedCheckboxCount = this.pitchDeskList.filter(
      (item) => item.isChecked === 1,
    ).length;
    this.selectAllCheckboxes =
      this.pitchDeskList.length > 0 &&
    ...
    if (isChecked) {
      ...
      this.exportDataIds = this.exportDataIds.filter(
  ...
  selectAllCheckbox() {
    ...
    this.selectedCheckboxCount = this.selectAllCheckboxes
      ? this.pitchDeskList.length
      : 0;
    this.exportDataIds = this.selectAllCheckboxes
      ? this.pitchDeskList.map((item) => item.id)
      : [];
  }
    ...
    if (this.exportCreditCount != 0) {
      ...
      if (this.exportDataIds.length == 0) {
        this.toast.add({
          severity: "error",
          summary: "error",
          detail: "Select Some data for export!.",
        });
        return;
      }
      if (
        ...
        if (this.exportCreditCount < this.exportDataIds.length) {
          this.toast.add({
            severity: "error",
            summary: "error",
            detail: "insufficient credits.Please Buy Some Credits.",
          });
          this.router.navigate(["/pages/export-credit"]);
          return;
        }
        ...
        if (this.exportCreditCount < this.exportDataIds.length) {
          this.toast.add({
            ...
            summary: "error",
            detail:
              "To download additional data beyond your free credits, please upgrade your plan.",
          });
          return;
        }
      }
