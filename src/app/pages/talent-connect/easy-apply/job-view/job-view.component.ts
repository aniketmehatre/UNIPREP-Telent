...
export class JobViewComponent implements OnInit {
      ...
      next: (response) => {
        ...
        if (response.job && response.job[0]) {
          ...
          this.jobDetails = {
            ...
            technical_proficiency: this.parseArrayData(
              jobData.technical_proficiency
            ),
            language_proficiency: this.parseLanguageProficiency(
              jobData.language_proficiency
            ),
            compensation_structure: this.getArrayFormat(
              jobData?.compensation_structure
            ),
  ...
  private parseLanguageProficiency(
    data: any
  ...
  shareQuestion(type: string) {
    ...
    if (domainName) {
      url = encodeURI(
        environment.jobDomain + `/view/${this.jobDetails?.uuid}/${domainName}`
      );
    } else {
  ...
  get shouldBlur(): boolean {
    const plan = this.authService._user?.current_plan_detail?.current_plan;
    return !plan || plan === "Standard";
  }

  get visibleEmail(): string {
    const email = this.jobDetails?.email ?? "";
    return this.shouldBlur ? this.obfuscateEmail(email) : email || "N/A";
  }
  ...
  copytextHandler(value: string, type: string): void {
    if (this.shouldBlur) {
      // show upgrade prompt / toast instead of copying
      // replace with your project's toast/modal method

      this.message.add({
        severity: "info",
        summary: "Upgrade required",
        detail: "Upgrade to view or copy contact details",
      });
      return;
    }
    this.copytext(value, type);
  }
}
