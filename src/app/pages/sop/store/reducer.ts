...
export const sopReducer = createReducer(
    ...
    on(loadUnivercityByCountrySuccess, (state, payload) => ({
        ...state,
        univercities: [{id: -1, name: 'Select', status: -1}, ...payload.data]
    })),

    on(checkPlagSuccess, (state, payload) => ({
        ...state,
        Plagiarism: payload.Plagiarism,
        document: payload.document,
        grammer: payload.grammer
    })),
    on(verifyPlagSuccess, (state, payload) => ({
        ...state,
        stage: payload.data.stage,
        document: payload.data.document,
