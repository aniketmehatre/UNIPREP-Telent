...
export class TestQuizService {
  ...
  getCategoryList(req: GetCategoriesPayload): Observable<CategoryResponse> {
    ...
    return this.http.get<CategoryResponse>(`${environment.ApiUrl}/getcareertoolcategorylist?module_id=${req.moduleId}`, {
      headers: headers
    });
  }
  getSubCategoryList(req: GetSubcategoryPayload): Observable<SubCategoryResponse> {
    const headers = new HttpHeaders().set("Accept", "application/json");
    return this.http.get<SubCategoryResponse>(environment.ApiUrl + `/getcareertoolcategorylisttwo?parent_category_id=${req.categoryId}&module_id=${req.moduleId}`, {
      headers: headers
    });
  }
  getQuizList(req: GetQuizPayload): Observable<QuizResponse> {
    const headers = new HttpHeaders().set("Accept", "application/json");
    return this.http.get<QuizResponse>(environment.ApiUrl + `/getcareertoolsubmodulelist?category_id=${req.categoryId}&module_type=${'Student'}`, {
      headers: headers
    });
  }
